...
class BeerGameAgent(object):
    ...
    def __init__(self, agentNum, IL, AO, AS, c_h, c_p, eta, compuType, config):
        ...
        if self.config.demandDistribution == 0:
            self.a_b = np.mean(
                (self.config.demandUp, self.config.demandLow)
            )  # parameters for the formula
            self.b_b = np.mean((self.config.demandUp, self.config.demandLow)) * (
                np.mean(
                    (
                        self.config.leadRecItemLow[self.agentNum],
                        self.config.leadRecItemUp[self.agentNum],
                    )
                )
                + np.mean(
                    (
                        self.config.leadRecOrderLow[self.agentNum],
                        self.config.leadRecOrderUp[self.agentNum],
                    )
                )
            )  # parameters for the formula
        elif (
            self.config.demandDistribution == 1
            or self.config.demandDistribution == 3
            or self.config.demandDistribution == 4
        ):
        ...
        self.hist = (
            []
        )  # this is used for plotting - keeps the history for only one game
        self.hist2 = []  # this is used for animation usage
        # self.srdqnBaseStock = (
        #     []
        # )  # this holds the base stock levels that srdqn has came up with. added on Nov 8, 2017
        self.T = 0
        self.bsBaseStock = 0
    ...
    def resetPlayer(self, T):
        ...
        self.AS = np.squeeze(
            np.zeros(
                (
                    1,
                    T
                    + max(self.config.leadRecItemUp)
                    + max(self.config.leadRecOrderUp)
                    + 10,
                )
            )
        )  # arriced shipment
        ...
        # )  # this holds the base stock levels that srdqn has came up with. added on Nov 8, 2017
        self.T = T
        self.curObservation = self.getCurState(
            1
    ...
    def actionValue(self, curTime, playType):
        if playType == "test":
            if self.config.fixedAction:
                ...
                #     a = max(
                #         0,
                #         self.config.actionList[np.argmax(self.action)]
                #         * self.config.action_step
                #         + self.AO[curTime],
                #     )
                if self.compTypeTest == "rnd":
                    a = max(
                        0,
                        self.config.actionList[np.argmax(self.action)]
                        + self.AO[curTime],
    ...
    def getReward(self):
        ...
        self.curReward = (
            self.c_p * max(0, -self.IL) + self.c_h * max(0, self.IL)
        ) / 200.0  # self.config.Ttest #
        self.curReward = -self.curReward
        # make reward negative, because it is the cost

        # sum total reward of each agent
        self.cumReward = self.cumReward + self.curReward
    ...
    def getCurState(self, t):
        if self.config.ifUseASAO:
            ...
            curState = np.array(
                [-1 * (self.IL < 0) * self.IL, 1 * (self.IL > 0) * self.IL, self.OO]
            )

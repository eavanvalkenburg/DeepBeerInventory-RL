...
class clBeerGame(object):
    def __init__(self, config):
        self.config = config
        self.curGame = (
            0  # The number associated with the current game (counter of the game)
        )
        self.curTime = 0
        self.totIterPlayed = (
            0  # total iterations of the game, played so far in this and previous games
        )
    ...
    def createAgent(self):
        ...
        return [
            Agent(
                i,
                self.config.ILInit[i],
                self.config.AOInit,
                self.config.ASInit[i],
                self.config.c_h[i],
                self.config.c_p[i],
                self.config.eta[i],
                agentTypes[i],
    ...
    def getTotRew(self):
        ...
        for i in range(self.config.NoAgent):
            self.players[i].curReward += self.players[i].eta * (
                totRew - self.players[i].cumReward
            )  # /(self.T)
    ...
    def distTotReward(self):
        ...
        for i in range(self.config.NoAgent):
            for j in range(self.T):
                if (
                    self.config.NoAgent > 1
                    and hasattr(self.players[i], "brain")
                    and (len(self.players[i].brain.replayMemory) > 0)
                ):
                    ...
                    self.players[i].brain.replayMemory[-1 * (j + 1)][2] += (
                        self.config.distCoeff / (self.config.NoAgent - 1)
                    ) * (
                        (totRew - self.players[i].cumReward) / (self.T)
                    )  # changes the last T periods in the replayMemory
    ...
    def getAction(self, k):
        ...
        if self.playType == "train":
            ...
            if self.players[k].compTypeTrain == "Strm":
                self.players[k].action = np.zeros(self.config.actionListLenOpt)
                self.players[k].action[
                    np.argmin(
                        np.abs(
                            np.array(self.config.actionListOpt)
                            - max(
                                0,
                                round(
                                    self.players[k].AO[self.curTime]
                                    + self.players[k].alpha_b
                                    * (self.players[k].IL - self.players[k].a_b)
                                    + self.players[k].betta_b
                                    * (self.players[k].OO - self.players[k].b_b)
                                ),
                            )
                        )
                    )
                            ...
                            np.argmin(
                                np.abs(
                                    ...
                                    - max(
                                        0,
                                        (
                                            self.players[k].int_bslBaseStock
                                            - (
                                                self.players[k].IL
                                                + self.players[k].OO
                                                - self.players[k].AO[self.curTime]
                ...
                raise Exception(
                    "The player type is not defined or it is not a valid type.!"
                )
        ...
        # get action for test runs
            ...
            #         self.players[k].action[
            #             int(self.loaded_dqn_actions[self.curTime])
            #         ] = 1
            #     else:
            #         self.players[k].action = self.players[k].brain.getDNNAction(
            #             self.playType
            #         )
            if self.players[k].compTypeTest == "Strm":
                self.players[k].action = np.zeros(self.config.actionListLenOpt)
    ...
    def next(self):
        ...
        leadTimeIn = randint(
            self.config.leadRecItemLow[self.config.NoAgent - 1],
            self.config.leadRecItemUp[self.config.NoAgent - 1],
        )
        # handle the most upstream recieved shipment
        self.players[self.config.NoAgent - 1].AS[
            self.curTime + leadTimeIn
        ] += self.players[self.config.NoAgent - 1].actionValue(
            self.curTime, self.playType
        )
        ...
        for k in range(self.config.NoAgent - 1, -1, -1):  # [3,2,1,0]
            ...
            possible_shipment = min(
                current_IL + self.players[k].AS[self.curTime],
                current_backorder + self.players[k].AO[self.curTime],
            )

            # plan arrivals of the items to the downstream agent
            if self.players[k].agentNum > 0:
                leadTimeIn = randint(
                    self.config.leadRecItemLow[k - 1], self.config.leadRecItemUp[k - 1]
                )
            ...
            self.players[k].nextObservation = self.players[k].getCurState(
                self.curTime + 1
            )
    ...
    def handelAction(self):
        ...
        leadTime = randint(
            self.config.leadRecOrderLow[0], self.config.leadRecOrderUp[0]
        )
        # set AO
        ...
        for k in range(0, self.config.NoAgent):
            ...
            self.players[k].hist += [
                [
                    ...
                    self.players[k].OO,
                    self.players[k].actionValue(self.curTime, self.playType),
                    self.players[k].curReward,
                    0,
                    # self.players[k].srdqnBaseStock[-1],
                ]
            ]

            # if (
            #     self.players[k].compTypeTrain == "srdqn" and self.playType == "train"
            # ) or (self.players[k].compTypeTest == "srdqn" and self.playType == "test"):
            #     self.players[k].hist2 += [
            #         [
            #             self.curTime,
            #             self.players[k].IL,
            #             self.players[k].OO,
            #             self.players[k].AO[self.curTime],
            #             self.players[k].AS[self.curTime],
            #             self.players[k].actionValue(self.curTime, self.playType),
            #             self.players[k].curReward,
            ...
            self.players[k].OO += self.players[k].actionValue(
                self.curTime, self.playType
            )  # open order level update
            leadTime = randint(
                self.config.leadRecOrderLow[k], self.config.leadRecOrderUp[k]
            )
            if self.players[k].agentNum < self.config.NoAgent - 1:
                self.players[k + 1].AO[self.curTime + leadTime] += self.players[
                    k
                ].actionValue(
                    self.curTime, self.playType
                )  # open order level update
    ...
    def playGame(self, demand, playType):
        ...
        while self.curTime <= self.T:
            ...
            # for k in range(0, self.config.NoAgent):
            #     if (
            #         self.players[k].compTypeTrain == "srdqn" and playType == "train"
            #     ) or (self.players[k].compTypeTest == "srdqn" and playType == "test"):
            #         # control the learner agent

            #         self.players[k].brain.train(
            #             self.players[k].nextObservation,
            #             self.players[k].action,
            #             self.players[k].curReward,
            #             self.curTime == self.T,
            #             self.playType,
    ...
    def getOptimalSol(self):
        ...
        if self.config.NoAgent != 1 and 1 == 2:
            ...
            for k in range(self.config.NoAgent - 1):
                if not (
                    self.players[k].c_h == self.players[k + 1].c_h
                    and self.players[k + 1].c_p == 0
                ):
            ...
            if self.ifOptimalSolExist == True:
                ...
                for k in range(self.config.NoAgent):
                    # DL_high
                    calculations[0][k] = (
                        (self.config.leadRecItemLow + self.config.leadRecItemUp + 2) / 2
                        + (self.config.leadRecOrderLow + self.config.leadRecOrderUp + 2)
                        / 2
                ...
                # S_high
                calculations[4] = np.round(
                    np.multiply(calculations[0], calculations[2])
                )
                # S_low
                calculations[5] = np.round(
                    np.multiply(calculations[0], calculations[3])
                )
                # S_avg
                ...
                for k in range(self.config.NoAgent):
                    if k == 0:
                        ...
                        self.players[k].bsBaseStock = (
                            calculations[6][k] - calculations[6][k - 1]
                        )
                        if self.players[k].bsBaseStock < 0:
                            self.players[k].bsBaseStock = 0
        elif self.config.NoAgent == 1:
            if self.config.demandDistribution == 0:
                self.players[0].bsBaseStock = (
                    np.ceil(
                        self.config.c_h[0]
                        / (self.config.c_h[0] + self.config.c_p[0] + 0.0)
                    )
                    * ((self.config.demandUp - self.config.demandLow - 1) / 2)
                    * self.config.leadRecItemUp
                )
        elif 1 == 1:
            f = self.config.f
    ...
    def doTestMid(self, demandTs):
            ...
            if self.config.ifPlaySavedData:
                hist2 = np.load(
                    os.path.join(
                        ...
                        "DQN-0-player-" + str(dnn_agent) + "-" + str(i) + ".npy",
                    )
                )
        ...
        if self.ifOptimalSolExist:
            ...
            print(
                "SUMMARY; {0:s}; ITER= {1:d}; SRDQN= [{2:s}]; SUM = {3:2.4f}; Rand= [{4:s}]; SUM = {5:2.4f}; STRM= [{6:s}]; SUM = {7:2.4f}; BS= [{8:s}]; SUM = {9:2.4f}".format(
            ...
            print(
                "SUMMARY; {0:s}; ITER= {1:d}; SRDQN= [{2:s}]; SUM = {3:2.4f}; Rand= [{4:s}]; SUM = {5:2.4f}; STRM= [{6:s}]; SUM = {7:2.4f}".format(
                    strftime("%Y-%m-%d %H:%M:%S", gmtime()),
                    self.curGame,
                    result_srdqn,
                    sum(resultSummary[0]),
    ...
    def doTest(self, m, demand):
        ...
        if (self.config.ifSaveFigure) and (
            self.curGame in range(self.config.saveFigInt[0], self.config.saveFigInt[1])
        ):
            plt.figure(
                self.curGame, figsize=(12, 8), dpi=80, facecolor="w", edgecolor="k"
            )
        ...
        if (
            (
                (self.config.ifSaveFigure)
                and (
                    self.curGame
                    in range(self.config.saveFigInt[0], self.config.saveFigInt[1])
                )
            )
            or (self.curGame >= self.config.maxEpisodesTrain - 1)
            or (len(self.middleTestResult) < self.config.testRepeatMid)
        ):

            # use random to get output.
            RsltRnd, plt = self.tester(
                ["rnd", "rnd", "rnd", "rnd"], plt, "y21", "RAND", m
            )

            # use formual to get output.
            RsltStrm, plt = self.tester(
                ["Strm", "Strm", "Strm", "Strm"], plt, "g", "Strm", m
            )

            # use optimal strategy to get output, if it works.
            if self.ifOptimalSolExist:
                if self.config.agentTypes == ["srdqn", "Strm", "Strm", "Strm"]:
                    Rsltbs, plt = self.tester(
                        ["bs", "Strm", "Strm", "Strm"], plt, "r", "Strm-BS", m
                    )
                elif self.config.agentTypes == ["Strm", "srdqn", "Strm", "Strm"]:
                    Rsltbs, plt = self.tester(
                        ["Strm", "bs", "Strm", "Strm"], plt, "r", "Strm-BS", m
                    )
                elif self.config.agentTypes == ["Strm", "Strm", "srdqn", "Strm"]:
                    Rsltbs, plt = self.tester(
                        ["Strm", "Strm", "bs", "Strm"], plt, "r", "Strm-BS", m
                    )
                elif self.config.agentTypes == ["Strm", "Strm", "Strm", "srdqn"]:
                    Rsltbs, plt = self.tester(
                        ["Strm", "Strm", "Strm", "bs"], plt, "r", "Strm-BS", m
                    )
                elif self.config.agentTypes == ["srdqn", "rnd", "rnd", "rnd"]:
                    Rsltbs, plt = self.tester(
                        ["bs", "rnd", "rnd", "rnd"], plt, "r", "RND-BS", m
                    )
                elif self.config.agentTypes == ["rnd", "srdqn", "rnd", "rnd"]:
                    Rsltbs, plt = self.tester(
                        ["rnd", "bs", "rnd", "rnd"], plt, "r", "RND-BS", m
                    )
                elif self.config.agentTypes == ["rnd", "rnd", "srdqn", "rnd"]:
                    Rsltbs, plt = self.tester(
                        ["rnd", "rnd", "bs", "rnd"], plt, "r", "RND-BS", m
                    )
                elif self.config.agentTypes == ["rnd", "rnd", "rnd", "srdqn"]:
                    Rsltbs, plt = self.tester(
                        ["rnd", "rnd", "rnd", "bs"], plt, "r", "RND-BS", m
                    )
                else:
                    Rsltbs, plt = self.tester(
                        ["bs", "bs", "bs", "bs"], plt, "r", "BS", m
                    )
        ...
        if self.config.ifSaveFigure and (
            ...
            savePlot(
                self.players, self.curGame, Rsltdnn, RsltStrm, Rsltbs, self.config, m
            )
        ...
        if self.ifOptimalSolExist:
            ...
            print(
                "output; {0:s}; Iter= {1:s}; SRDQN= [{2:s}]; sum = {3:2.4f}; Rand= [{4:s}]; sum = {5:2.4f}; Strm= [{6:s}]; sum = {7:2.4f}; BS= [{8:s}]; sum = {9:2.4f}".format(
                    strftime("%Y-%m-%d %H:%M:%S", gmtime()),
                    str(str(self.curGame) + "-" + str(m)),
                    result_srdqn,
                    sum(Rsltdnn),
                    result_rand,
                    sum(RsltRnd),
            ...
            print(
                "output; {0:s}; Iter= {1:s}; SRDQN= [{2:s}]; sum = {3:2.4f}; Rand= [{4:s}]; sum = {5:2.4f}; Strm= [{6:s}]; sum = {7:2.4f}".format(
                    strftime("%Y-%m-%d %H:%M:%S", gmtime()),
                    str(str(self.curGame) + "-" + str(m)),
                    result_srdqn,
                    sum(Rsltdnn),
    ...
    def tester(self, testType, plt, colori, labeli, m):
        ...
        if (
            ...
            plt = plotting(
                plt,
                [np.array(self.players[i].hist) for i in range(0, self.config.NoAgent)],
                colori,
                labeli,
            )
        if self.config.ifsaveHistInterval and (
            (self.curGame == 0)
            or (self.curGame == 1)
            or (self.curGame == 2)
            or (self.curGame == 3)
            or ((self.curGame - 1) % self.config.saveHistInterval == 0)
            or ((self.curGame) % self.config.saveHistInterval == 0)
            or ((self.curGame) % self.config.saveHistInterval == 1)
            ...
            for k in range(0, self.config.NoAgent):
                ...
                np.save(
                    os.path.join(self.config.model_dir, name),
                    np.array(self.players[k].hist2),
                )
    ...
    def update_OO(self):
        for k in range(0, self.config.NoAgent):
            if k < self.config.NoAgent - 1:
                self.players[k].OO = sum(self.players[k + 1].AO) + sum(
                    self.players[k].AS
                )
            else:
